...
class AiNewsNode:
    def __init__(self,llm):
        """
        Initialize the AINewsNode with apikeys for groq and tavitly
    ...
    def fetch_news(self,state: dict) -> dict:
        """
        fetch_news news based on specified frequency 
        Args:
           state(dict): The state dictionary containing 'frequency'
        Returns:
            dict:Updated state with 'news_data' key containing fetched news
        """
        self.state['frequency'] =state['messages'][0].content.lower()
        time_range_map= {'daily':'d','weekly':'w','monthly':'m','yearly':'y'}
        days_map={'daily':1, 'weekly':7, 'monthly':30, 'yearly':366,}
        response = self.tavily.search(
            query="Top Artificial Intelligence (AI) technology news India and globally",
            topic="news",
            time_range=time_range_map[self.state['frequency']],
            include_answer="advanced",
            max_results=20,
            days=days_map[self.state['frequency']],
